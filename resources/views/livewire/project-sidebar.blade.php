...
>
    <div class="flex items-center mb-6">
        ...
        <button wire:click="openTerminal" title="Open Terminal" class="text-gray-400 hover:text-white">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 28"
                 stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="mb-4">
        <div class="flex border-b border-gray-700">
            <button @click="activeTab = 'project'" :class="{ 'border-b-2 border-blue-500': activeTab === 'project' }"
                    class="py-2 px-4 text-sm font-medium text-gray-300 hover:text-white">
                Project
            </button>
            <button @click="activeTab = 'files'" :class="{ 'border-b-2 border-blue-500': activeTab === 'files' }"
                    class="py-2 px-4 text-sm font-medium text-gray-300 hover:text-white">
                Files
            </button>
            <button @click="activeTab = 'snapshots'"
                    :class="{ 'border-b-2 border-blue-500': activeTab === 'snapshots' }"
                    class="py-2 px-4 text-sm font-medium text-gray-300 hover:text-white">
                Snapshots
            </button>
            <button @click="activeTab = 'settings'" :class="{ 'border-b-2 border-blue-500': activeTab === 'settings' }"
                    class="py-2 px-4 text-sm font-medium text-gray-300 hover:text-white">
                Settings
            </button>
        </div>
    ...
    <div x-show="activeTab === 'files'">
                ...
                @foreach($files as $index => $file)
                    <li class="flex items-center justify-between bg-gray-700 p-2 rounded">
                        ...
                        <div class="flex items-center">
                            <button wire:click="openFile('{{ $file['full_path'] }}')"
                                    class="text-gray-400 hover:text-white mr-2" title="Open file">
        ...
        <!-- File Browser Modal -->
        <div x-show="showFileBrowser" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full"
                ...
                <div class="mt-3 text-center">
                    ...
                    <div class="mt-2 px-7 py-3">
                        ...
                        @if($currentPath !== $project->full_path)
                            <button wire:click="browseFiles('{{ dirname($currentPath) }}')"
                        ...
                        <div class="mt-4 space-y-2 text-left max-h-96 overflow-y-auto">
                            <ul>
                                @foreach($directoryContents as $item)
                                    <li>
                                        @if($item['type'] === 'dir')
                                            <button wire:click="browseFiles('{{ $item['path'] }}')"
                                                    class="text-blue-400 hover:underline">
                                                📁 {{ $item['name'] }}
                                            </button>
                                        @else
                                            <button wire:click="addFile('{{ $item['path'] }}')"
                    ...
                    <div class="items-center px-4 py-3">
                        <button @click="toggleFileBrowser()"
    ...
    <div x-show="activeTab === 'snapshots'">
        ...
        <button wire:confirm wire:click="createSnapshot"
                class="bg-blue-600 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out transform">
            New Snapshot
        </button>
        ...
        @forelse($snapshots as $snapshot)
            <div class="flex items-center justify-between p-4 rounded-lg mb-2">
                ...
                <div>
                    <button wire:confirm="Are you sure? All changes done after this snapshot will be lost."
                            wire:click="restoreSnapshot({{ $snapshot->id }})"
                            class="bg-sky-600 hover:bg-sky-800 text-white font-bold py-0 px-2 rounded transition duration-300 ease-in-out transform">
                        Restore
                    </button>
                </div>
            </div>
    ...
    <div x-show="activeTab === 'settings'">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-300">Input Tokens: {{ Number::format($inputTokens) }}
                ({{ Number::currency($inputCost) }})</p>
            <p class="text-gray-300">Output Tokens: {{ Number::format($outputTokens) }}
                ({{ Number::currency($outputCost) }})</p>
        ...
        <div class="mb-6">
            <button wire:click="deleteProject" wire:confirm="Delete this project and all its files?"
                    class="bg-red-600 hover:bg-red-800 text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out transform">
                Delete Project
            </button>
        </div>
    </div>
